package com.zsgf.platform.model.entity.yearPlan;

import com.baomidou.mybatisplus.annotation.TableName;
import com.fasterxml.jackson.annotation.JsonFormat;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Getter;
import lombok.Setter;
import lombok.ToString;

import javax.validation.constraints.Digits;
import javax.validation.constraints.Size;
import java.io.Serializable;
import java.math.BigDecimal;
import java.util.Date;

/**
 * 危险废物_管理计划_05危险废物委托利用处置对象 pp_wxfw_year_plan_waste_entrust_use_disposal
 *
 * @author gzl
 * @date 2023-02-09
 */
@Setter
@Getter
@ToString
@ApiModel("危险废物_管理计划_05危险废物委托利用处置")
@TableName("pp_wxfw_year_plan_waste_entrust_use_disposal")
public class YearPlanWasteEntrustUseDisposal implements Serializable {
    private static final long serialVersionUID = 1L;

    /**
     * 主键
     */
    @ApiModelProperty(value = "主键")
    @Size(max = 100, message = "主键最多输入100个字符")
    private String id;

    /**
     * 基本信息表主键
     */
    @ApiModelProperty(value = "基本信息表主键")
    @Size(max = 100, message = "基本信息表主键最多输入100个字符")
    private String mainid;

    /**
     * 危险废物产生概况信息主键
     */
    @ApiModelProperty(value = "危险废物产生概况信息主键")
    @Size(max = 100, message = "危险废物产生概况信息主键最多输入100个字符")
    private String childid;

    /**
     * 委外单位的企业id
     */
    @ApiModelProperty(value = "委外单位的企业id")
    @Size(max = 100, message = "委外单位的企业id最多输入100个字符")
    private String dwid;

    /**
     * 危险废物委托利用处置单位名称
     */
    @ApiModelProperty(value = "危险废物委托利用处置单位名称")
    @Size(max = 200, message = "危险废物委托利用处置单位名称最多输入200个字符")
    private String czdwmc;

    /**
     * 许可证主键
     */
    @ApiModelProperty(value = "许可证主键")
    @Size(max = 100, message = "许可证主键最多输入100个字符")
    private String xkzid;

    /**
     * 许可证编号
     */
    @ApiModelProperty(value = "许可证编号")
    @Size(max = 200, message = "许可证编号最多输入200个字符")
    private String xkzbh;

    /**
     * 利用处置方式大类
     */
    @ApiModelProperty(value = "利用处置方式大类")
    @Size(max = 200, message = "利用处置方式大类最多输入200个字符")
    private String lyczfs;

    /**
     * 利用处置方式小类
     */
    @ApiModelProperty(value = "利用处置方式小类")
    @Size(max = 200, message = "利用处置方式小类最多输入200个字符")
    private String lyczfsxl;

    /**
     * 上年度实际委托利用处置量
     */
    @ApiModelProperty(value = "上年度实际委托利用处置量")
    @Digits(integer = 15, fraction = 4, message = "上年度实际委托利用处置量整数最多15位，小数最多4位")
    private BigDecimal sndsjwtlyczl;

    /**
     * 本年度计划委托利用处置量
     */
    @ApiModelProperty(value = "本年度计划委托利用处置量")
    @Digits(integer = 15, fraction = 4, message = "本年度计划委托利用处置量整数最多15位，小数最多4位")
    private BigDecimal bndsjwtlyczl;

    /**
     * 所属省
     */
    @ApiModelProperty(value = "所属省")
    @Size(max = 40, message = "所属省最多输入40个字符")
    private String sssheng;

    /**
     * 所属市
     */
    @ApiModelProperty(value = "所属市")
    @Size(max = 40, message = "所属市最多输入40个字符")
    private String ssshi;

    /**
     * 所属区
     */
    @ApiModelProperty(value = "所属区")
    @Size(max = 40, message = "所属区最多输入40个字符")
    private String ssqu;

    /**
     * 修改时间
     */
    @ApiModelProperty(value = "修改时间")
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
    private Date modifyrq;

}
