...
public class CustomProcessDiagramCanvas {
    ...
    public CustomProcessDiagramCanvas(int width, int height, int minX, int minY, String imageType, String activityFontName, String labelFontName, ClassLoader customClassLoader) {
        if (minX < 0) {
            this.canvasWidth = width + Math.abs(minX);
        } else {
            this.canvasWidth = width;
        }
        if (minY < 0) {
            this.canvasHeight = height + Math.abs(minY);
        } else {
    ...
    public void initialize(String imageType) {
        ...
        if (minX < 0) {
            g.translate(Math.abs(minX), 0);
        }
        if (minY < 0) {
            g.translate(0, Math.abs(minY));
        }
...
//		LABEL_FONT = new Font(labelFontName, Font.ITALIC, 10);
    ...
    public void drawArrowHead(Line2D.Double line, double scaleFactor) {
        ...
        transformation.translate(x2, y2);
    ...
    public void drawMultiInstanceMarker(boolean sequential, int x, int y, int width, int height) {
        ...
        int lineY = y + height - rectangleHeight - 5;
        ...
        if (sequential) {
            ...
            g.draw(new Line2D.Double(lineX + 2, lineY, lineX + 2, lineY + rectangleHeight - 2));
            g.draw(new Line2D.Double(lineX + rectangleWidth / 2, lineY, lineX + rectangleWidth / 2, lineY + rectangleHeight - 2));
            g.draw(new Line2D.Double(lineX + rectangleWidth - 2, lineY, lineX + rectangleWidth - 2, lineY + rectangleHeight - 2));
        }
