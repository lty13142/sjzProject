 ...
 */
@Slf4j
public class ProjectSentinelInvocationHandler implements InvocationHandler {
	...
	ProjectSentinelInvocationHandler(Target<?> target, Map<Method, InvocationHandlerFactory.MethodHandler> dispatch,
			...
			if (methodMetadata == null) {
				...
				try {
					...
					if (fallbackFactory != null) {
						...
						// 若是R类型 执行自动降级返回R
						if (RestResult.class == method.getReturnType()) {
							log.error("feign 服务间调用异常", ex);
							return RestResult.failed(HttpStatus.HTTP_INTERNAL_ERROR,ex.getLocalizedMessage());
						}
						else {
							throw ex;
						}
	...
	public boolean equals(Object obj) {
		if (obj instanceof SentinelInvocationHandler) {
			ProjectSentinelInvocationHandler other = (ProjectSentinelInvocationHandler) obj;
