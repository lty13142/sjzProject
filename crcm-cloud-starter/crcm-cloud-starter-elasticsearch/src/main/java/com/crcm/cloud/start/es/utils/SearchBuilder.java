package com.crcm.cloud.start.es.utils;
...
import com.crcm.core.response.PageResult;
...
public class SearchBuilder {
    ...
    public PageResult<JsonNode> getPage(Integer page, Integer limit) throws IOException {
        ...
        return PageResult.<JsonNode>builder().records(list).current(page).total(totalCnt).size(limit).build();
    }
    ...
    private List<JsonNode> getList(SearchHits searchHits) {
        ...
        if (searchHits != null) {
            searchHits.forEach(item -> {
                JsonNode jsonNode = JSONObject.parseObject(item.getSourceAsString(), JsonNode.class);
