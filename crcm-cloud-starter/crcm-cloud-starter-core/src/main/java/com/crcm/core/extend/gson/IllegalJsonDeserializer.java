...
/**
 * @ClassName IllegalJsonDeserializer
 * @Description 不规范Json数据Gson反序列化,使用方法：在需要使用的类上面添加@JsonAdapter(value = IllegalJsonDeserializer.class)
...
//            }
    ...
    private List<BoundField> getFields(Type typeOfT) {
        ...
        for (Field field : fields) {
            ...
            result.add(new BoundField(field, fieldNames, getSelect(field)));
        }
        return result;
    }
    ...
    private class BoundField {
        ...
        private BoundField(Field field, List<String> alternates, String select) {
            this.select = select;
            this.field = field;
