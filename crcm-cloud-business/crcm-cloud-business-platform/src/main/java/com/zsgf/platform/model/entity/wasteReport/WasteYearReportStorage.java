...
/**
 * 危险废物_产生年报_05贮存情况对象 pp_wxfw_waste_year_report_storage
 *
 * @author gzl
 * @date 2023-02-09
 */
...
@Getter
@ToString
@ApiModel("危险废物_产生年报_05贮存情况")
@TableName("pp_wxfw_waste_year_report_storage")
public class WasteYearReportStorage implements Serializable {
     ...
     * 年底贮存数量
     */
    @ApiModelProperty(value = "年底贮存数量")
    @Digits(integer = 15, fraction = 4, message = "年底贮存数量整数最多15位，小数最多4位")
    private BigDecimal nbsl;

    /**
     * 上年度贮存数量
     */
    @ApiModelProperty(value = "上年度贮存数量")
    @Digits(integer = 15, fraction = 4, message = "上年度贮存数量整数最多15位，小数最多4位")
    private BigDecimal sndnbsl;
